...
class Solver():
    ...
    def __init__(self, args, train_data, val_data, writer):
        ...
        if self.resume:
            ...
            self.model = tf.keras.models.load_model(self.opt['resume_path'], custom_objects={'tf': tf, 'charbonnier_loss': charbonnier_loss, 'peak_signal_noise_ratio': peak_signal_noise_ratio})
            ...
            p_model = tf.keras.models.load_model(self.opt['qat_path'], custom_objects={'tf': tf, 'charbonnier_loss': charbonnier_loss, 'peak_signal_noise_ratio': peak_signal_noise_ratio})
            ...
            self.lg.info('Load from checkpoint: [{}]'.format(self.opt['pretrain_path']))
            self.model = tf.keras.models.load_model(self.opt['pretrain_path'], custom_objects={'tf': tf, 'charbonnier_loss': charbonnier_loss, 'peak_signal_noise_ratio': peak_signal_noise_ratio})
    ...
    def train(self):
        if self.resume == False:
            if self.opt['loss'] == 'cbl':
                self.model.compile(optimizer=self.optimizer, loss=charbonnier_loss, metrics=[peak_signal_noise_ratio])
            else:
                self.model.compile(optimizer=self.optimizer, loss=self.opt['loss'], metrics=[peak_signal_noise_ratio])
    ...
    def scheduler(self, epoch):
        if 'lr_policy' in self.opt:
            ...
            lr_min = self.opt['lr_minimum']
            lr_max = self.opt['lr']

            change_point = int(0.05 * T_max)
            if epoch <= change_point:
                current_lr = K.get_value(self.model.optimizer.lr)
                next_lr = (epoch / change_point) * current_lr

            else:
                next_lr = lr_min + 0.5 * (lr_max - lr_min) * (1 + math.cos((epoch / T_max) * math.pi))
...
class Epoch_End_Callback(Callback):
    ...
    def on_epoch_end(self, epoch, logs):
        ...
        self.lg.info('Epoch: {:4} | PSNR: {:.4f} | Loss: {:.4f} | lr: {:.2e} | Best_PSNR: {:.4f} in Epoch [{}]'.format(epoch, psnr, loss, K.get_value(self.model.optimizer.lr), self.best_psnr, self.best_epoch))
